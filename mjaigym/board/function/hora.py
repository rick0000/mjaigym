...
class PointDatam:
    def __init__(self, fu, fan, oya, hora_type):
        self.fu = fu
        self.fan = fan
        self.oya = oya
        ...
        if self.fan >= YAKUMAN_FAN:
            ...
            self.fan >= 5
            or (self.fan >= 4 and self.fu >= 40)
            or (self.fan >= 3 and self.fu >= 70)
        ):
...
class Candidate:
    ...
    def from_already_splited_chitoitsu(
        ...
        return HoraInfo(fu=0, fan=0, yakus=[], points=0, oya_payment=0, ko_payment=0)
    ...
    def __init__(self, hora_yakuinfo, combination, taken_index):
        ...
        if self.combination == "chitoitsu":
            ...
            for mentsu_type, mentsu_pais in self.combination:
                num_this_taken = len(
                    [
                        p
                        for p in mentsu_pais
                        if self.hora_yakuinfo.taken.is_same_symbol(p)
                    ]
                )
                has_taken = (
                    self.taken_index >= total_taken
                    and self.taken_index < total_taken + num_this_taken
                )
                if mentsu_type == "toitsu":
                    if self.janto:
                        raise Exception("should not happen")
                    self.janto = Mentsu(
                        pais=mentsu_pais, type="toitsu", visibility=None
                    )
                else:
                    min_an = (
                        "min"
                        if (has_taken and self.hora_yakuinfo.hora_type == "ron")
                        else "an"
                    )
                    self.mentsus.append(
                        Mentsu(
                ...
                if has_taken > 0:
                    if mentsu_type == "toitsu":
                        ...
                        if self.hora_yakuinfo.taken.is_same_symbol(mentsu_pais[1]):
                            ...
                            mentsu_pais[0].number == 1
                            and self.hora_yakuinfo.taken.number == 3
                        ) or (
                            mentsu_pais[0].number == 7
                            and self.hora_yakuinfo.taken.number == 7
                        ):
                            self.machi = "penchan"
                        else:
                            self.machi = "ryanmen"
        ...
        datum = PointDatam(
            self.fu, self.fan, self.hora_yakuinfo.oya, self.hora_yakuinfo.hora_type
        )
    ...
    def valid(self):
        return (
            len([y for y in self.yakus if y[0] not in ["dora", "uradora", "akadora"]])
            > 0
        )
    ...
    def get_yakus(self):
        if (
            self.hora_yakuinfo.first_turn
            and self.hora_yakuinfo.hora_type == "tsumo"
            and self.hora_yakuinfo.oya
        ):
            self.add_yaku("tenho", YAKUMAN_FAN, 0)
        if (
            self.hora_yakuinfo.first_turn
            and self.hora_yakuinfo.hora_type == "tsumo"
            and not self.hora_yakuinfo.oya
        ):
        ...
        self.add_yaku(
            "dora", self.hora_yakuinfo.num_doras, self.hora_yakuinfo.num_doras
        )
        self.add_yaku(
            "uradora", self.hora_yakuinfo.num_uradoras, self.hora_yakuinfo.num_uradoras
        )
        self.add_yaku(
            "akadora", self.hora_yakuinfo.num_akadoras, self.hora_yakuinfo.num_akadoras
        )
    ...
    def num_anko(self):
        return len(
            [
                m
                for m in self.mentsus
                if ((m.type in ["kotsu", "kantsu"]) and (m.visibility == "an"))
            ]
        )
        ...
        return (
            len(
                [
                    m
                    for m in self.mentsus
                    if m.pais[0].is_wind() and m.type in ["kotsu", "kantsu"]
                ]
            )
            == 4
        )
        ...
        is_3_wind = (
            len(
                [
                    m
                    for m in self.mentsus
                    if m.type in ["kotsu", "kantsu"] and m.pais[0].is_wind()
                ]
            )
            == 3
        )
    ...
    def pinfu(self):
        return (
            all([m.type == "shuntsu" for m in self.mentsus])
            and self.machi == "ryanmen"
            and self.fanpai_fan(self.janto.pais[0]) == 0
        )
    ...
    def jikaze(self):
        return any(
            [
                m.type in ["kotsu", "kantsu"]
                and m.pais[0].str == self.hora_yakuinfo.jikaze
                for m in self.mentsus
            ]
        )
    ...
    def bakaze(self):
        return any(
            [
                m.type in ["kotsu", "kantsu"]
                and m.pais[0].str == self.hora_yakuinfo.bakaze
                for m in self.mentsus
            ]
        )
        ...
        for i in range(len(same_types)):
            ...
            if (
                any(
                    [
                        m.pais[0].type == "m"
                        and m.pais[0].number == target_start_number
                        for m in same_types
                    ]
                )
                and any(
    ...
    def ikkitsukan(self):
        ...
        for mps in ["m", "p", "s"]:
            ...
            if (
                any([s.pais[0].number == 1 for s in target_shuntsus])
                and any([s.pais[0].number == 4 for s in target_shuntsus])
                and any([s.pais[0].number == 7 for s in target_shuntsus])
            ):
                return True
        return False
    ...
    def honchantaiyao(self):
        return all(
            [any([p.is_yaochu() for p in m.pais]) for m in self.mentsus + [self.janto]]
        )
    ...
    def junchantaiyao(self):
        return all(
            [
                m.pais[0].type in ["m", "p", "s"]
                and any([p.number in [1, 9] for p in m.pais])
                for m in self.mentsus + [self.janto]
            ]
        )
    ...
    def ryanpeko(self):
        ...
        for m1 in self.mentsus:
            if (
                any(
                    [
                        m2 is not m1 and m2.pais[0].is_same_symbol(m1.pais[0])
                        for m2 in self.mentsus
                    ]
                )
                == False
            ):
    ...
    def num_sangenpais(self):
        return len(
            [
                m
                for m in self.mentsus
                if ((m.type in ["kotsu", "kantsu"]) and (m.pais[0].is_sangenpai()))
            ]
        )
...
class Hora:
    def __init__(
        ...
        num_same_as_taken = len(
            [f for f in self.free_pais if self.taken.is_same_symbol(f)]
        )
