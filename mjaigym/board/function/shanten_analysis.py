...
class ShantenAnalysis():
                    ...
                    for type in MENTSU_TYPES:
                        ...
                        if self.max_shanten == -1:
                            if type in ['ryanpen', 'kanta']:
                                continue
                            if any([m[0] == 'toitsu' for m in mentsus]) and type == 'toitsu':
                                continue

                        (removed_pais, remains_set) = self.remove(pai_set, type, first_pai)
                        if remains_set is not None:
                            (shanten, combinations) = self.count_normal(remains_set, mentsus + [[type, removed_pais]])
                            if shanten < min_shanten:
                                min_shanten = shanten
                                min_combinations = combinations
                                if self.need_all_combinations == False and min_shanten == -1:
                                    break
    ...
    def get_min_shanten_for_mentsus(self, mentsus):
        ...
        if 'toitsu' in mentsu_categories:
            ...
            min_shantens.append(self.get_min_shanten_without_janto(mentsu_categories, num_remain_pais))
        else:
            min_shantens.append(self.get_min_shanten_without_janto(mentsu_categories, num_remain_pais) + 1)
            if num_remain_pais >= 2:
                min_shantens.append(self.get_min_shanten_without_janto(mentsu_categories, num_remain_pais - 2))
