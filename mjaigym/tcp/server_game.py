...
class ServerGame:
    ...
    def is_ok_end(self):
        return (
            ...
            and len(self.board.dealer_history) > 0
            and self.board.dealer_history[-1]["type"] == MjMove.end_game.value
        )
    ...
    def message_in_view(self, message, player_id):
        ...
        if message["type"] == MjMove.start_game.value:
            ...
            for i in range(4):
                if i != player_id:
                    rewritee_message["tehais"][i] = ["?"] * len(
                        rewritee_message["tehais"][i]
                    )
    ...
    def dump_mjson(self, dir_path):
        ...
        with open(fpath, "wt") as f:
            formatted_lines = [
                json.dumps(l) + os.linesep for l in self.board.dealer_history
            ]
    ...
    def dump_error_mjson(self, dir_path):
        ...
        with open(fpath, "wt") as f:
            ...
            f.write(f"---\n")
            formatted_responses = [
                json.dumps(l) + os.linesep for l in self.board.renponse_history
            ]
